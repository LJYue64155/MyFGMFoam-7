...
:
    ...
    rho_(const_cast<volScalarField&>(this->thermo().rho())),           //--- senbin                  
...
hashedWordList FGMModel<ReactionThermo>::tables()
{
    ...
    tableNames.append("rho"); //--- Read rho table
    ...
    if(this->active())
    {
       ...
       forAll(ZmeanCells, cellI)    // 1st: for internal field
       {
	        ...
	        //- find up-bound and pos for FGM table interpolation
             ...
             rhoCells[cellI] = solver_.interpolate(ubParams4dIF_, posParams4dIF_, 4); //--- by Liu, Update density from table
	         sourceYcCells[cellI] = solver_.interpolate(ubParams4dIF_, posParams4dIF_, 5);
	     
            if (useProgressVariableVariance_)
            {
                YWICells[cellI] = solver_.interpolate(ubParams4dIF_, posParams4dIF_, 6);
                YuWICells[cellI] = solver_.interpolate(ubParams4dIF_, posParams4dIF_, 7);
                YbWICells[cellI] = solver_.interpolate(ubParams4dIF_, posParams4dIF_, 8);
            }
	        else
            {
                YWICells[cellI] = 0.0;
...
FGMModel<ReactionThermo>::R  //- Fuel consumption rate matrix.
(
...
/*     // 获取当前热力学模型的引用
    ...
    // 获取所有组分场的列表 (例如 Y["CH4"], Y["O2"], Y["CO2"] 等)
    const PtrList<volScalarField>& Y = thermo.composition().Y();
    
    // 2. 定义你需要考虑源项的组分名称列表
    // 注意：这里的名字必须与 thermophysicalProperties 中的 species 完全一致！
    wordList targetSpecies({"CH4", "O2", "CO"}); // 举例：只考虑甲烷、氧气和一氧化碳

    // 3. 遍历所有的组分
    forAll(Y, i)
    {
        // 获取当前遍历到的组分名称
        const word& speciesName = thermo.composition().species()[i];
    } */
...
FGMModel<ReactionThermo>::Qdot() const
{
    ...
    // Qdot_ = this->thermo().h() * this-> * sourceYc_;  // the sourceYc_ is in unit of kmol/m3/s, and h is in unit of J/kg. So the unit of Qdot_ is W/m3.
